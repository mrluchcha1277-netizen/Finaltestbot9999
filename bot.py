...
def sanitize_channel_input(text: str) -> str:
    """
    @username / https://t.me/username / numeric ID → send_message এর জন্য chat_id।
    """
    text = text.strip()
    if text.startswith("@"):
        return text
    if "t.me/" in text:
        ...
        if username:
            if not username.startswith("@"):
                username = "@" + username
            return username
    return text  # ধরে নিলাম ID


def sanitize_verification_chat(text: str) -> str:
    """
    verification link থেকে chat_id বানায় (get_chat_member এর জন্য)।
    """
    text = text.strip()
    if text.startswith("@"):
        return text
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Bot OFF এবং user admin না হলে → limited access
    if not bot_is_on(context) and not is_admin(user.id, context):
        ...
        await update.message.reply_text(
            ...
            f"For any concerns or support, please contact the admin:\n👉 @{SUPER_ADMIN_USERNAME}"
        )
        return

    # Admin initialize / normal start
    if ensure_admin(update, context):
        status = "ON ✅" if bot_is_on(context) else "OFF ❌"
        text = (
            "💐🌟🎉 WELCOME TO YOUR BOT 🎉🌟💐\n\n"
            f"👑 Admin: @{user.username or SUPER_ADMIN_USERNAME}\n"
        ...
        await update.message.reply_text(
            f"Hi Dear @{user.username or 'User'}\n"
            f"UserID: {user.id}\n\n"
            "This is the Power Point Break Giveaway Bot.\n"
...
async def panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "/approve - Approve & publish winners\n"
        "/reject  - Reject current winners\n"
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔁 BOT ON/OFF", callback_data="panel_toggle")],
    ]
...
async def panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel_new":
        await query.message.reply_text("📝 To start a new giveaway, please type:\n/newgiveaway")
    elif data == "panel_toggle":
        on = not bot_is_on(context)
        context.bot_data["bot_on"] = on
...
async def cg_winner_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode != "AUTO":
        await update.message.reply_text("❌ Only AUTO mode supported.\nSend: AUTO")
        return CG_WINNER_MODE
    g["winner_mode"] = "AUTO"
    await update.message.reply_text(
...
async def cg_ban_old_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "BAN":
        ...
        await update.message.reply_text(
            ...
            "Format:\n@Username/UserID\n\n"
            "Example:\n@User/123456\n\n"
            "You can send multiple lines together.\n"
...
async def cg_old_winner_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    OLD winners list একবারে অনেক লাইন হিসেবে নিতে পারবে।
    প্রতিটি লাইন: @Username/UserID
    """
    g = context.user_data["new_giveaway"]
    text = update.message.text.strip()
    ...
    # old_winners সেট না থাকলে বানিয়ে নি
    if "old_winners" not in g:
        g["old_winners"] = set()

    added = 0
    skipped: List[str] = []

    # মেসেজের সব লাইন একসাথে প্রসেস
    ...
    if added == 0:
        await update.message.reply_text(
            "❌ No valid entries found.\nUse format: @Username/UserID\nor send DONE."
        )
    else:
        msg = f"✔ Saved {added} old winner(s).\nSend more list, or DONE."
        if skipped:
            msg += (
                "\n\nSome lines were skipped (wrong format):\n" +
                "\n".join(skipped[:10])
            )
...
async def cg_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Here is the preview of your Giveaway:\n\n" + preview_text)
    await update.message.reply_text(
...
async def cg_confirm_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info("Starting countdown job…")
    job_queue.run_repeating(
        countdown_job,
        interval=g["interval_seconds"],  # 1 / 3 / 5 sec
        first=0,                         # সাথে সাথেই শুরু
...
async def join_giveaway_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query:
        return
    logger.info("JOIN GIVEAWAY callback received.")
    await query.answer()
    ...
    if not bot_is_on(context):
        await query.reply_text(
            ...
            f"Please contact the admin: @{SUPER_ADMIN_USERNAME}"
        )
        return
...
async def countdown_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not giveaway or giveaway.get("status") != "running":
        ...
        logger.info("No running giveaway, stopping countdown job.")
        return
    ...
    if time_left <= 0:
        ...
        try:
            ...
            logger.exception(f"Failed to edit message at end: {e}")
        ...
        if super_admin_id:
            try:
                ...
                await context.bot.send_message(
                    ...
                    text=(
                        "🏁🔥 GIVEAWAY CLOSED!\n"
                        f"🔍 {len(winners)} Lucky Winner(s) selected…\n\n"
                        f"Preview:\n\n{preview}\n\n"
                        "Send /approve to publish or /reject to cancel."
                    ),
                )
            except Exception as e:
                logger.exception(f"Failed to send approval request: {e}")
    ...
    try:
        ...
        logger.info("Countdown message updated.")
    except Exception as e:
        logger.exception(f"Failed to update countdown message: {e}")
...
async def reject_winners_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.exception("Failed to send cancellation message")
